...
```
## 2.3 Con trỏ hằng – Constant pointer
Con trỏ hằng sẽ bao gồm các đặc điểm:
+ Không thể thay đổi địa chỉ đang trỏ tới.
+ Có thể đọc ghi dữ liệu đang trỏ tới.  
Ứng dụng con trỏ hằng để lưu trữ địa chỉ một thanh ghi, người dùng chỉ thao tác với các giá trị trên thanh ghi đó. Việc sử dụng thanh ghi sẽ giúp tốc độ chạy chương trình nhanh hơn.
```c
int value = 5;
int anothervalue = 10;
...
ptr_const = &anothervalue; // lỗi (bản thân con trỏ là một hằng)
```
##  2.4 Hằng con trỏ - Pointer to constant 
Hằng con trỏ có các đặc điểm sau:
+ Có thể thay đổi địa chỉ đang lưu trữ.
+ Không được thay đổi dữ liệu đang trỏ tới.  
Ứng dụng hằng con trỏ để bảo vệ những dữ liệu quan trọng, không muốn bị tác động trong quá trình thực thi chương trình.
```c
int value = 5;
int anothervalue = 10;

const* int const_ptr = &value; // *const_ptr = 5

*const_ptr = 10 // lỗi (dữ liệu đang trỏ tới một là một hằng số)
```
##  2.5 Con trỏ NULL
